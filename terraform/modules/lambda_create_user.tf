module "create_user_function" {
  ...
  source_path = "${path.module}/../../../backend/create_user"
  ...
  allowed_triggers = {
    source_arn = "arn:aws:execute-api:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:${module.runa_vault_api.api_id}/*/*/create_user"
  }
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "RunaVault_Lambda_CreateUser"
    }
  )
}
