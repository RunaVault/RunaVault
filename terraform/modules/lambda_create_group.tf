module "create_group_function" {
  source = "../modules/lambda"

  function_name = "create_group"
  description   = "Lambda function to create user"

  source_path = "${path.module}/../../../backend/create_group"
  ...
  allowed_triggers = {
    source_arn = "arn:aws:execute-api:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:${module.runa_vault_api.api_id}/*/*/create_group"
  }
  ...
  tags = merge(
    local.common_tags,
    {
      Name = "RunaVault_Lambda_CreateGroup"
    }
  )
}
